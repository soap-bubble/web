...
const nextSceneIdFromCast = ({ param1: nextSceneId1, nextSceneId }) => nextSceneId || nextSceneId1;

function findCastsWithChangeSceneType(scene) {
  return scene.casts
    .filter(isChangeSceneType);
}

function findCastsWithNextSceneId(scene) {
  return scene.casts
    .filter(isSceneWithNextScene);
}
...
function withScenes(scenes) {
  const selfie = {
    ...
    nextChildren(scene) {
      ...
      return hotspotSceneChilds.concat(nextSceneChilds)
        .map(nextSceneIdFromCast);
    },
    addChildren(parent, node) {
      ...
      const children = selfie.nextChildren(node)
        .map(nextSceneIdFromCast);
