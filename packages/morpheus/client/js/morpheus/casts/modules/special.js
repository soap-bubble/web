...
// const movExt = isSafari ? 'mp4' : 'webm';
...
function generateImages({ images, dimensions, gamestates }) {
  const { width, height } = dimensions
...
function generateControlledFrames({ controlledCasts, dimensions, gamestates }) {
  const { width, height } = dimensions
...
const selectors = memoize(scene => {
  ...
  const selectMovieCasts = createSelector(
    selectAllMovieCasts,
    extraCasts => extraCasts.filter(c => !c.audioOnly && !c.image),
  )
  ...
  const selectSoundCasts = createSelector(
    ...
    (extraCasts, controlledCasts) =>
      controlledCasts.concat(extraCasts).filter(c => c.audioOnly),
  )
...
export const delegate = memoize(scene => {
  ...
  function updateAssets({ getState, autoplay, assets, movieCasts, dispatch }) {
  ...
  function doEnter({ setState, images, controlledCasts, assets, movies }) {
    return (dispatch, getState) => {
  ...
  function doUnload({ assets, videoPreloads }) {
    return () => {
