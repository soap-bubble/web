...
define({
  ...
  twitchHooks(app, logger) {
    ...
    app.get('/twitch/hook/:topic', (req, res) => {
      ...
      try {
        const { query: { 'hub.challenge': challenge } } = req;
        if (challenge) {
  ...
  provideTwitchUserToken(logger, profileProvider, saveProfile, twitchClientId, twitchClientSecret) {
    ...
    async function provideTwitchUserToken() {
      ...
      if (typeof isExpired === 'undefined') {
        try {
          const  { data: { valid } } = await tokenStatus();
          isExpired = !valid;
          if (valid) {
            setExpireTimeout();
          }
...
function createApi(twitchClientId, twitchWebhookEndpoint, twitchSecret, twitchLogin, logger, provideTwitchUserToken, twitchHooks) {
  ...
  async function promiseClientHeaders() {
    return {
      'Client-ID': twitchClientId,
    };
  }
...
// https://api.twitch.tv/helix/users/follows?first=1&to_id=
  const api = {
    ...
    async subscribe(topicName, topicUrl) {
      ...
      try {
        const { status, data } = await attemptWithRefreh(async () => await axios.post(twitchApiNew('webhooks/hub'), {
    ...
    async unsubscribe(topics) {
      await Promise.all(topics.map(async (topic) => {
        ...
        const { data } = await attemptWithRefreh(async () => await axios.post(twitchApiNew('webhooks/hub'), {
          hub: {
    ...
    async getMyStream() {
      try {
        ...
        const { data } = await attemptWithRefreh(async () => await axios.get(twitchApiNew('streams'), {
          headers: await promiseNewAuthorizatedHeaders(),
          params: {
    ...
    async getMyUserId() {
      try {
        ...
        const { data } = await attemptWithRefreh(async () => await axios.get(twitchApiNew('users'), {
          headers: await promiseNewAuthorizatedHeaders(),
          params: {
            login: twitchLogin,
          },
    ...
    async getTags(userId) {
      try {
        logger.info('getTags');
        const { data } = await attemptWithRefreh(async () => await axios.get(twitchApiNew('streams/tags'), {
          headers: await promiseNewAuthorizatedHeaders(),
          params: {
    ...
    async setTags(userId, tags) {
      try {
        logger.info('setTags');
        await attemptWithRefreh(async () => await axios.put(twitchApiNew(`streams/tags?broadcaster_id=${userId}`), {
          tag_ids: tags,
        }, {
          headers: await promiseNewAuthorizatedHeaders(),
        }));
    ...
    async findAllStreamTags(findTags) {
      ...
      try {
        ...
        let differs;
        do {
          const { data: { data, pagination: nextPage } } = await attemptWithRefreh(async () => await axios.get(twitchApiNew(`tags/streams`), {
            headers: await promiseNewAuthorizatedHeaders(),
            params: {
              first: 100,
              ...pagination ? {
        ...
        logger.error(message, err);
        logger.info(tags.map(tag => `${tag.tag_id} (${tag.localization_names['en-us']})`).join(', '))
        return {
          error: message,
        };
      }
