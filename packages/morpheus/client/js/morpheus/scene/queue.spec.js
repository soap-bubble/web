...
describe('Scene queue', () => {
  ...
  it('can cancel', () => {
    ...
    const task1Start = jest.fn().mockReturnValue(
      new Promise(resolve => {
        resolveTask1Start = resolve
      }),
    )
    const task1End = jest.fn().mockReturnValue(
      new Promise(resolve => {
  ...
  it('error', async () => {
    ...
    try {
      await queue.add({
        id: 0,
        tasks: [() => Promise.reject(new Error('foo')), job1End],
      })
    } catch (e) {
      didThrow = true
      expect(() => {
