...
// These do not need to be ported
...
// createEpic(action$ =>
//   action$.ofType('AUTO_SCENE_WAIT').forEach(({ payload: sceneId, cb }) => {
...
//   action$.ofType('AUTO_SAVE').forEach(({ cb }) => {
//     cb({
...
// createEpic((action$, { dispatch }) =>
//   action$.ofType('AUTO_LOAD').forEach(({ payload, cb }) => {
