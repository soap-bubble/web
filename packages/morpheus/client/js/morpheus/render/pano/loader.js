...
import raf from 'raf'
import { isEnabledCast } from '../../casts/matchers'
import {
  PANO_CHUNK,
  DST_RATIO,
  DST_WIDTH,
...
const filterPanoCastData = scene =>
  get(scene, 'casts', []).find(c => c.__t === 'PanoCast')
const filterPanoAnimData = scene =>
  get(scene, 'casts', []).filter(c => c.__t === 'PanoAnim')
...
export default ({ scene, canvasTexture, onVideoEndFactory }) => {
  ...
  return {
    ...
    load(gamestates, volume) {
      const panoCastData = filterPanoCastData(scene)
      const videoCastsData = filterPanoAnimData(scene).filter(
        cast =>
          !existsInAssets(cast) &&
          isEnabledCast({
            cast,
