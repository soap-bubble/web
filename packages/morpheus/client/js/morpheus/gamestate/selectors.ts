...
export const gamestates = createSelector(root, r => r.get('idMap'))

export function castEnabled({ comparators }: any) {
  return createSelector(gamestates, _gamestates =>
...
export const forState = createSelector(gamestateMap, gs => ({
  byId(id: number | null): Gamestate {
    const gamestate = gs.get(id)
    if (!gamestate) {
      throw new Error(`Gamestate with id ${id} not found`)
    }
    return {
      get value() {
